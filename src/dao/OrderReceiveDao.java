...
public class OrderReceiveDao extends Dao {
	...
	public List<OnlineOrder> filter(Product product, User user,Date orderTime) throws Exception {
		...
		if (product == null && user != null && orderTime == null) {
		    condition = "where user_name=? receive = true";
		    System.out.println("1");
		}
		// product のみ設定されている場合の条件
		else if (product != null && user == null && orderTime == null) {
		    condition = "where product_name=? receive = true";
		    System.out.println("2");
		}
		// orderTime のみ設定されている場合の条件
		else if (product == null && user == null && orderTime != null) {
		    condition = "where order_time=? receive = true";
		    System.out.println("3");
		}
		// user と product が設定されている場合の条件
		else if (product != null && user != null && orderTime == null) {
		    condition = "where user_name=? and product_name=? receive = true";
		    System.out.println("4");
		}
		// user と orderTime が設定されている場合の条件
		else if (product == null && user != null && orderTime != null) {
		    condition = "where user_name=? and order_time=? receive = true";
		    System.out.println("5");
		}
		// product と orderTime が設定されている場合の条件
		else if (product != null && user == null && orderTime != null) {
		    condition = "where product_name=? and order_time=? receive = true";
		    System.out.println("6");
		}
		// すべてが設定されている場合の条件
		else if (product != null && user != null && orderTime != null) {
		    condition = "where user_name=? and product_name=? and order_time=? receive = true";
		    System.out.println("7");
		}
		// それ以外の場合（条件なし）
		else {
		    condition = "where receive = true";
